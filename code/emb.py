...
if __name__ == "__main__":
    ...
    #   data = next(data_iter) #data = data_iter.next()
    ...
    for class_id, group_data in data_groups.items():
        ...
        imgslist, captionslist, cap_lenslist, class_idslist, keyslist = [],[],[],[],[]
        for img, caption, cap_len, key in group_data:
            print(type(caption))
            imgslist.append(img)
            captionslist.append(caption)
            cap_lenslist.append(cap_len)
            keyslist.append(key)

        hidden = text_encoder.init_hidden(len(imgslist))
        words_embs, sent_emb = text_encoder(captionslist, cap_lenslist, hidden)
        words_embs, sent_emb = words_embs.detach(), sent_emb.detach()

        indexes = CoresetGreedy.CoresetGreedy(words_embs).sample(0.1)
        coreset_list.append(indexes)
        print(indexes)
